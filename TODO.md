...
| OMO-004 | P1 / evaluation work ready | Implement held-out helper-selection, argument-fidelity, result-interpretation and routing-contention evaluation; expand helper coverage only after protected evidence and calibration. | Typed proposals and the shared OMO-012 contract are complete. Corpus expansion and human-reviewed evidence remain. |
...
| OMO-014 | P1 / workflow scaffolding ready; activation blocked | Add a manual-only protected GitHub Actions training workflow. Require exact code, dataset and base-model revisions; explicit method/hardware/timeout/max-cost inputs; concurrency cancellation; dependency locks; checkpoint/resume; Trackio and a dry-run mode. Use GitHub Actions as the control plane and an isolated Hugging Face Job as the initial execution backend; do not add a schedule, PR trigger or automatic promotion. | Workflow and preflight implementation can proceed with execution disabled. Activation depends on OMO-006, OMO-008 and OMO-013 plus owner-approved spend, HF Jobs eligibility, a protected `training` environment and narrowly scoped credentials. |
